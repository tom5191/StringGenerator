...
const { generate, generateMultiple } = require('../src/index');
const { createPool } = require('../src/services/generator');
const unsafeUrlChars = ['?', '>', '<', '=', '+', '&', '^', '%', '`'];

function match(string, { upperCase, lowerCase, numbers, specialChars }) {
...
function notMatch(string, { upperCase, lowerCase, numbers, specialChars, unsafeUrl }) {
...
describe('string generator', function () {
  ...
  describe('generate()', function () {
    ...
    it('should create a url safe string with all options required', async function () {
      ...
      const string = await generate(options)
      match(string, {
        lowerCase: true, upperCase: true, numbers: true, specialChars: true, specialChars: true
      })
